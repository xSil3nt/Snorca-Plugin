...
ConfigOptionDef plugin_def_to_host_def(const PluginConfigOptionDef &plugin_def)
{
    ...
    if (!plugin_def.category.empty())
        def.category = plugin_def.category;
    ...
    if (!plugin_def.default_string.empty()) {
        ...
        def.set_default_value(new ConfigOptionBool(plugin_def.default_int != 0));
    }
...
void ConfigSchemaRegistry::register_preset_options()
{
    std::lock_guard<std::mutex> lock(m_mutex);
    for (const PluginConfigRegistration &reg : m_registrations) {
        switch (reg.preset_type) {
        case PluginPresetType::Print:
            Preset::register_print_option(reg.def.opt_key);
            break;
        case PluginPresetType::Filament:
            Preset::register_filament_option(reg.def.opt_key);
            break;
        case PluginPresetType::Printer:
        case PluginPresetType::PhysicalPrinter:
            Preset::register_printer_option(reg.def.opt_key);
            break;
        default:
            break;
        }
